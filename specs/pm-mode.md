...
接口：**`/pm` 是原 `coding-agent chat`（ReplSession）的一个模式**，不是新 agent——输入 `/pm` 切换 persona（`apply_pm_mode(player)` 设置 `agent._system_prompt_override`），再 `/pm` 回到普通模式；PM 步骤命令 `/vision /story /mvp /validate /polish /pitch` 已有 `COMMANDS` 注册（`/` 菜单可补全），在 pm 模式下经 `agent/pm.py` 的模块函数 `pm_step` 驱动，**支持流式输出**（`stream=True` 时经 `on_delta` 逐 token 打印，不重复二次输出）。命令后的文字作为用户消息传入（如 `/vision 我的产品…`）。交付物写 `<workdir>/demo/{DEMO_SPEC, DEMO_SCRIPT, PITCH}.md` 与 demo 代码。
