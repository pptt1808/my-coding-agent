...
- R16（状态提示 / spinner）交互会话显示 Claude-Code 风格的**状态行**：agent 思考/执行工具期间，终端始终显示动画 spinner（如 `⠋ 思考中`、`⠙ 读取 report.py`、`执行 edit_file`），避免模型不回 token 时屏幕静止；真实输出（token）到达时先用 `\r 清行` 覆盖状态行再打印，返回最终答案时清空状态行。由 `agent/spinner.py::StatusSpinner` 驱动，经 `CodingAgent._on_status` 回调触发；
