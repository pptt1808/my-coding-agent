...
- R12（指令菜单）输入 `/`（单独）显示全部可用命令；`/前缀` 唯一匹配时自动展开执行（如 `/comp`→`/compact`），多匹配提示歧义，未知命令列出可用命令；**全角 `／`（中文输入法）按 `/` 处理**；`/` 作为**真实字符**回显，可随时用 Backspace 删除以**收起菜单**（用 ANSI 清除菜单所在的多行），回车提交命令时也会先收起菜单；
