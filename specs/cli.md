...
- R12（指令菜单 / 补全）`/` 作为**真实字符**回显；交互编辑器采用**整屏重绘**（同 prompt_toolkit / OpenCode 之法）：每次按键从稳定原点半屏重绘，仅在**当前输入以 `/` 开头**时，在输入行下方画出**补全候选**（按已输入前缀过滤），否则不画。因此删除 `/` → 下一帧菜单自动消失；回车提交命令前也先收起菜单。`/前缀` 唯一匹配时自动展开执行（如 `/comp`→`/compact`），多匹配提示歧义，未知命令列出可用命令；**全角 `／`（中文输入法）按 `/` 处理**；
